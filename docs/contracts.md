...
| `POST /vitals/arm` | -> `{"armed_at": "...", "pending": true, "window_s": 120}`. The camera screen calls this when Start is pressed. The presage-worker in `--watch` mode polls `GET /vitals/arm` (same body) every 2 s and runs one capture per distinct `armed_at`; `pending` stays true while the arm is younger than `window_s` and no vitals row has been received since it. `DELETE /vitals/arm` -> `{"armed_at": null, "pending": false, "window_s": 120, "note": null}` (Cancel). `PATCH /vitals/arm` `{"note": "<=300 chars", "final": false}` is the worker telling the phone why a capture failed (webcam busy, no face, face lost); `note` comes back on every GET until the next POST/DELETE, and `final: true` also ends the arm (`armed_at: null, pending: false`) so the screen stops waiting and shows the note. `worker_seen_at` (H34, every response; `null` until a worker has polled): the server time of the last `GET /vitals/arm` carrying the header `x-scallion-worker: watch` or the last `PATCH` for that user. The watcher sends the header on every request and keeps polling while a capture child runs, so a stamp older than ~15 s (compared with `armed_at` plus the time since Start, both server clock) means no worker is running for that account and the camera screen ends the wait with that message instead of a generic timeout. The worker's retry after a fixable failure runs at once and does not depend on `pending` (the 120 s window is only for starting the first attempt); a Cancel or a new Start calls it off. Transient per-process state, no table; the worker's `SCALLION_API_TOKEN` list must include the phone's account. |
