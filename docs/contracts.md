...
**v12 (Sun H32, D): `/vitals/arm` (POST, GET, DELETE) pairs the phone's Start with the laptop worker: the camera screen arms, the presage-worker `--watch` polls and captures once per arm. H33: `PATCH /vitals/arm` + `note` so the worker can tell the phone why a capture failed. Additive; `/vitals` and `/vitals/latest` unchanged.**
