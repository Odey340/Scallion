...
**v12 (Sun H32, D): `/vitals/arm` (POST, GET, DELETE) pairs the phone's Start with the laptop worker: the camera screen arms, the presage-worker `--watch` polls and captures once per arm. Additive; `/vitals` and `/vitals/latest` unchanged.**
...
| `POST /vitals/arm` | -> `{"armed_at": "...", "pending": true, "window_s": 120}`. The camera screen calls this when Start is pressed. The presage-worker in `--watch` mode polls `GET /vitals/arm` (same body) every 2 s and runs one capture per distinct `armed_at`; `pending` stays true while the arm is younger than `window_s` and no vitals row has been received since it. `DELETE /vitals/arm` -> `{"armed_at": null, "pending": false, "window_s": 120}` (Cancel). Transient per-process state, no table; the worker's `SCALLION_API_TOKEN` list must include the phone's account. |
