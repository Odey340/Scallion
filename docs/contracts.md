...
**v12 (Sun H32, D): `/vitals/arm` (POST, GET, DELETE) pairs the phone's Start with the laptop worker: the camera screen arms, the presage-worker `--watch` polls and captures once per arm. H33: `PATCH /vitals/arm` + `note` so the worker can tell the phone why a capture failed. H34: `worker_seen_at` on the arm status (stamped by worker requests carrying `x-scallion-worker`) so the phone can say "no laptop worker is polling" within 20 s. Additive; `/vitals` and `/vitals/latest` unchanged.**
...
| `POST /vitals/arm` | -> `{"armed_at": "...", "pending": true, "window_s": 300}`. The camera screen calls this when Start is pressed. The presage-worker in `--watch` mode polls `GET /vitals/arm` (same body) every 2 s and runs one capture per distinct `armed_at`; `pending` stays true while the arm is younger than `window_s` and no vitals row has been received since it. `DELETE /vitals/arm` -> `{"armed_at": null, "pending": false, "window_s": 300, "note": null}` (Cancel; the camera screen also sends it when it gives up waiting). `PATCH /vitals/arm` `{"note": "<=300 chars", "final": false, "armed_at": "<the arm this is about>"}` is the worker telling the phone why a capture failed (webcam busy, no face, face lost); a note whose `armed_at` is not the current arm (the phone cancelled or pressed Start again meanwhile) is dropped, so a stale verdict cannot end the newer Start (`armed_at` omitted: applies to whatever is current, pre-H34); `note` comes back on every GET until the next POST/DELETE, and `final: true` also ends the arm (`armed_at: null, pending: false`) so the screen stops waiting and shows the note. `worker_seen_at` (H34, every response; `null` until a worker has polled): the server time of the last `GET /vitals/arm` carrying the header `x-scallion-worker: watch` or the last `PATCH` for that user. The watcher sends the header on every request and keeps polling while a capture child runs, so a stamp older than ~15 s (compared with `armed_at` plus the time since Start, both server clock) means no worker is running for that account and the camera screen ends the wait with that message instead of a generic timeout. The worker's retry after a fixable failure runs at once and does not depend on `pending`; a Cancel or a new Start calls it off. `window_s` is 300 (H34, was 120) so a Start placed while the laptop is busy with a 3-minute attempt is still pending when it frees up. Transient per-process state, no table; the worker's `SCALLION_API_TOKEN` list must include the phone's account. |
