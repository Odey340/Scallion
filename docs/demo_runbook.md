...
| Camera pulse | Before judging: `cd presage-worker && node index.mjs --watch` and leave it; close Teams/Zoom and anything else using the laptop webcam. Pressing Start on the phone arms `/vitals/arm`; the watcher captures (45-60 s) and posts to every account in `SCALLION_API_TOKEN`, so the phone can be on the demo account or the presenter's. If a capture fails the phone shows the laptop's reason (webcam busy, no face); if no watcher is polling for the phone's account it says so within 20 s. A slow face lock is fine: the screen waits up to ~3 min per attempt and the laptop retries once. **After any web deploy, reload the phone's tab** (the app shows a "Scallion was updated, Reload" bar; a tab from before the deploy runs the old flow and never reaches the laptop). No timing to get right. | `node index.mjs --watch --replay test/fixtures/capture_real.json` (leave it running; each Start on the phone posts the real Saturday capture, 68.5 bpm) | Yes, real capture replayed |
