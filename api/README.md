...
Live mode needs `GEMINI_API_KEY`; `/health` reports `"gemini": "live"|"fake"`, `"auth": "jwt"|"bypass"` and `"db": "tiger"|"memory"` (vitals fall back to an in-memory store without `TIGER_DATABASE_URL`).
